...
def chat_alert():
    ...
    text, ferr = _safe_invoke(llm.open_alert_chat, cid, client, verdict, profile)
    if ferr:
...
def chat_start():
    ...
    cid = _ensure_chat_id()
    text, ferr = _safe_invoke(llm.open_start_chat, cid, client, verdict, profile)
    if ferr:
        return jsonify(ok=False, error=ferr), 502
    return jsonify(ok=True, chat_id=cid, text=text)
...
def chat_message():
    ...
    cid = _ensure_chat_id()
    result, ferr = _safe_invoke(llm.continue_chat, cid, client, text, lat, lon)
    if ferr:
        return jsonify(ok=False, error=ferr), 502
